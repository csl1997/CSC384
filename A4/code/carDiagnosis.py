...
if __name__ == '__main__':
    ...
    v = st
    print('V0 = ' + v.name)
    print('P(V0) = ' + str(VE(car, v, [])[0] * 100))
    lst = []
    num = 1
    for t in [al, ba, bv, mf, ds]:
        lst.append(t)
        t.set_evidence(t.domain()[0])
        print("Evidences{}: {} = {}".format(num, t.name, t.get_evidence()))        
        probs = VE(car, v, lst)
        print('Probility of V0: ' + str(probs))
        doms = v.domain()
        print()
        num += 1
    ...
    lst = []
    num = 1
    for t in [bv, ss, tm, asys, fs]:
        lst.append(t)
        t.set_evidence(t.domain()[1])
        print("Evidences{}: {} = {}".format(num, t.name, t.get_evidence()))        
        probs = VE(car, v, lst)
        print('Probility of V0: ' + str(probs))
        doms = v.domain()
